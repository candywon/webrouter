...
def create_alias():
    ...
    if not data or not data.get('alias') or not data.get('target'):
        return jsonify({'error': 'alias 和 target 字段必填'}), 400
    ...
    if existing:
        return jsonify({'error': f'别名 {data["alias"]} 已存在，请用 PUT 更新'}), 409
    ...
    return jsonify({'message': '模型别名创建成功', 'alias': data['alias'], 'target': data['target']}), 201
...
def update_alias(alias_name):
    ...
    if not data or not data.get('target'):
        return jsonify({'error': 'target 字段必填'}), 400
    ...
    _notify_proxy_reload()
    return jsonify({'message': '模型别名更新成功', 'alias': alias_name, 'target': data['target']})
