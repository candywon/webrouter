...
> 一站式 AI API 网关管理：多源聚合、健康监控、成本计费、团队协作

## 功能概览

|| 模块 | 功能 |
|------|------|
| 数据源管理 | 注册直连/聚合/自建等多种 Provider，统一纳管所有 API 源 |
| 仪表盘 | 系统概览、请求统计、Provider 健康一览 |
| 渠道管理 | Provider 下渠道详情、负载均衡、优先级（newapi/oneapi 类型） |
| 健康监控 | 实时检测 Provider 可用性、延迟追踪、自动降级 |
| 告警中心 | 自定义告警规则、多级通知、历史记录 |
| 成本计费 | 用量统计、额度管理、费用报表 |
| 团队管理 | 成员邀请、角色权限、配额分配 |
| CLI 生成 | 一键生成渠道配置命令行 |
| 系统设置 | Provider 连接、告警配置、系统参数 |
...
| `aggregate` | 聚合平台（AnyRoute/OhMyGPT/API2D） | 健康 + 延迟 + 手动成本 |
| `newapi` | 自建 New-API 实例 | 完整（渠道+用户+日志+计费） |
| `oneapi` | 自建 One-API 实例 | 完整（渠道+用户+日志） |
...
└─────────────┘               │    :5000         │
                     ...
                     │               │               │
              ┌──────▼──────┐ ┌──────▼──────┐ ┌──────▼──────┐
              │   direct    │ │  aggregate  │ │   newapi    │
              │  (官方直连)  │ │  (聚合平台)  │ │ (自建网关)   │
              │  HTTP only  │ │  HTTP only  │ │ DB + HTTP   │
              └─────────────┘ └─────────────┘ └──────┬──────┘
                                                     │
                                              ┌──────▼──────┐
                                              │ New-API (Go)│
...
WebRouter 通过 Provider 抽象层管理所有 API 源，New-API 是功能最丰富的 Provider 类型（DB直读），其他类型通过 HTTP 检测纳入统一管理。
